# -*- coding: utf-8 -*-
# internet announcement Global Plugin for NVDA
# Copyright (C) 2023 hamid rezayi <hamidrehzaey>
# This file is covered by the GNU General Public License.
# You can read the licence by clicking Help->Licence in the NVDA menu
# or by visiting http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
# Shortcut: NVDA+Alt+i
#it  has been directly copied from the dropbox addon
# copyright (C) hamid rezayi
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	scriptCategory = u"internet status"
	...
	def _findInternetAccess(self):
		...
		for trayIcon in objects:
			...
			if iconName is not None and "internet access" in iconName.lower() or iconName is not None and "not connected - no connections are available" in iconName.lower() or iconName is not None and "not connected - connections are available" in iconName.lower():
				# internet object found
				return trayIcon
		return None

	def script_announceInternetAccess(self, gesture):
		trayIcon = self._findInternetAccess()
		if not trayIcon:
			# Translators: the message presented when internet tray icon was not found
			ui.message(_("internet  icon not found"))
			return

		name = trayIcon.name.split()
		repeatCount = scriptHandler.getLastScriptRepeatCount()
		if repeatCount == 0:
			# announce internet state
#			if win11_22h2:
#				del(name[0:3])
#			del(name[1])
			name = " ".join(name)
			...
			if (
				(currentProcess.lower() == u'internet' and objFocused.windowClassName.lower() == u'#32768')
				...
				try:
					...
					# Translators: the message reported when it is not possible to activate internet context menu.
					ui.message(_("Unable to activate internet context menu"))
	# Translators: message presented when user performes input help for the Shift+NVDA+i script
	script_announceInternetAccess.__doc__ = _(
		"If pressed once, announces internet status. "
		"If pressed twice, open the internet context menu by clicking on its systray icon"
	)

	__gestures = {
		"kb:NVDA+alt+i": "announceInternetAccess",
	}
